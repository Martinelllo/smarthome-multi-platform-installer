...
class RotaryControls(UIControls):
    def __init__(self):
        ...
        # detect rotation
        self.pigpio.callback(self.next_gpio, pigpio.EITHER_EDGE,  lambda gpio, level, time: self.__trigger_rotary(self.next_func, time))
        self.pigpio.callback(self.prev_gpio, pigpio.EITHER_EDGE,  lambda gpio, level, time: self.__trigger_rotary(self.prev_func, time))
