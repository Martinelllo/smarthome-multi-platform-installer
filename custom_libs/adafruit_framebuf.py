...
class GS2HMSBFormat:
    ...
    @staticmethod
    def rect(framebuf: 'FrameBuffer', x, y, width, height, color):
...
class RGB565Format:
    ...
    def set_pixel(self, framebuf: 'FrameBuffer', x, y, color):
        """Set a given pixel to a color."""
        index = (y * framebuf.stride + x) * 2
...
class BitmapFont:
    ...
    def __init__(self, font_name="font5x8.bin"):
        ...
        try:
            ...
            get_logger().error("Could not find font file", font_name)
            raise
